 ...
 * Body: { phone, address }
 * Optional: multipart field "image" to update the profile picture
 */
router.put("/update_profile/:id", verifyToken, upload.single("image"), async (req, res) => {
  try {
    const { phone, address } = req.body;

    if (req.file) {
      await pool.query(
        "UPDATE employees SET phone=?, address=?, image=? WHERE id=?",
        [phone, address, req.file.filename, req.params.id]
      );
      return res.json({ Status: true, message: "Profile updated", image: req.file.filename });
    }
