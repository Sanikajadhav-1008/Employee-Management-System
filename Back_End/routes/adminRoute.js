 ...
 * POST /auth/register
 * Body: { email, password }
 * Public self-registration for a new admin account.
 */
router.post("/register", async (req, res) => {
  try {
    ...
    if (password.length < 6)
      return res.json({ Status: false, Error: "Password must be at least 6 characters" });
    ...
    console.error(err);
    res.json({ Status: false, Error: "Failed to register admin" });
  }
});
